...
export const getMonthlyReport = catchAsync(async (req, res, next) => {
  ...
  // Check cache
  const cacheKey = `attendance_monthly_${month}_${userType}_${classId || 'all'}_${sectionId || 'all'}`;
...
export const getDailyReport = catchAsync(async (req, res, next) => {
  ...
  // Check cache
  const cacheKey = `attendance_daily_${date}_${userType}_${classId || 'all'}_${sectionId || 'all'}`;
  if (shortCache.has(cacheKey)) {
    return res.status(200).json({
      status: "success",
      source: "cache",
